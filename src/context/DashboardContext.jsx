...
export function DashboardProvider({ children }) {
  ...
  const updateOnboardingSession = useCallback(async (sessionId, step, data) => {
    ...
    setState(s => ({
      ...
      onboardingSessions: s.onboardingSessions.map(ses =>
        ses.id === sessionId
          ? { ...ses, step, ...data, lastActivityAt: now, status: "in_progress" }
          : ses
      ),
    }));
  ...
  const submitOnboarding = useCallback(async (sessionId, data) => {
    ...
    setState(s => ({
      ...
      onboardingSessions: s.onboardingSessions.map(ses =>
        ses.id === sessionId
          ? { ...ses, ...data, status: "submitted", submittedAt: now, lastActivityAt: now }
          : ses
      ),
    }));
  ...
  const createUser = useCallback(async (user) => {
    ...
    const full = {
      ...
      note: user.note || "",
      password: user.password || "",   // ← preserve password for dashboard login
    };
