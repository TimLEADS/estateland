...
export default function PaymentPage() {
  ...
  // Auto-redirect to Stripe when we have valid data (no coupon step, no extra click)
  useEffect(() => {
    if (!data?.plan?.id || !data?.contact?.name?.trim() || !data?.contact?.email?.trim() || paymentLoading || redirectStarted.current) return;
    ...
    <div style={{ minHeight: "100vh", background: T.bg, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 520, margin: "0 auto", padding: "clamp(48px, 8vw, 80px) 24px 120px" }}>
        ...
        <p style={{ fontFamily: font.body, fontSize: 16, color: T.mute, marginBottom: 40, lineHeight: 1.6 }}>
          Sending you to secure checkout…
        </p>
        ...
        <section style={sectionStyle}>
          ...
          {paymentLoading && <p style={{ fontFamily: font.body, fontSize: 14, color: T.mute }}>Redirecting to payment…</p>}
          <Link to="/get-started" style={{ fontFamily: font.body, fontSize: 14, fontWeight: 600, color: T.mute, padding: "18px 24px", textDecoration: "none", display: "inline-flex", alignItems: "center", marginTop: 16 }}>
            ← Back to Get started
          </Link>
        </section>
      </div>
