    ...
    (async () => {
      try {
        ...
        if (res.ok && data.payment_status === "paid") {
          ...
          addPayment({
            stripeSessionId: data.id, amountTotal: data.amount_total,
            currency: (data.currency || "usd").toUpperCase(),
            customerEmail: data.customer_email, planId: data.metadata?.planId
          });
          if (!recordedUserRef.current) {
            ...
            if (sub && !existingUser) {
              ...
              createUser({
                name: c.name || "", email: c.email || data.customer_email || "",
                phone: c.phone || "", planId: sub.plan?.id || data.metadata?.planId || "",
                region: sub.territory?.region || "", zips: sub.territory?.zips || "",
                state: c.state || "", primaryArea: c.primaryAreas || "",
                primarySMR: c.radius || "", secondaryArea: c.secondaryAreas || "",
                secondarySMR: "", leadType: c.leadType || "", note: c.note || "",
            ...
            {PRICING_PLANS.map((p) => {
              ...
              return (
                <button key={p.id} type="button" onClick={() => setSelectedPlan(p)}
                  style={{ width: "100%", textAlign: "left", padding: "24px 28px", borderRadius: 16,
