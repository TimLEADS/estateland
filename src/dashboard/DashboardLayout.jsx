...
const sidebarLinks = [
  ...
  { to: "/dashboard/email", label: "📧 Email Center", adminOnly: true },
...
export default function DashboardLayout() {
  ...
  return (
        <div style={{ minHeight: "100vh", background: C.surface, position: "relative", display: "flex", flexDirection: "column" }}>
                <div style={{ position: "absolute", inset: 0, background: "url(https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=1920&q=80) center/cover no-repeat", opacity: 0.08, pointerEvents: "none", zIndex: 0 }} aria-hidden="true" />
                <style>{`
                        @import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;1,400&family=Plus+Jakarta+Sans:wght@300;400;500;600;700&display=swap');
                              `}</style>style>
      <header style={{ position: "relative", zIndex: 1, height: 64, background: "rgba(8,8,8,0.85)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
                        <button type="button" onClick={() => setSidebarOpen((o) => !o)} aria-label="Toggle sidebar" style={{ width: 40, height: 40, borderRadius: 8, border: `1px solid ${C.border}`, background: "transparent", color: C.cream, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>&#9776;</button>button>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                        ...
                        <button type="button" onClick={handleLogout} style={{ fontFamily: font.body, fontSize: 12, fontWeight: 500, color: C.mute, background: "transparent", border: `1px solid ${C.border}`, padding: "8px 16px", borderRadius: 8, cursor: "pointer" }}>Log out</button>button>
              </div>div>
      </header>header>
                    ...
                    <aside style={{ width: 260, background: "rgba(8,8,8,0.75)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", borderRight: `1px solid ${C.border}`, padding: "24px 0", flexShrink: 0 }}>
                                <nav style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                                  {navLinks.map((link) => {
                                      ...
                                      return (
                                                          <Link key={link.to} to={link.to} style={{ fontFamily: font.body, fontSize: 13, fontWeight: 500, color: active ? C.gold : C.creamDim, padding: "12px 24px", textDecoration: "none", borderLeft: active ? `3px solid ${C.gold}` : "3px solid transparent", background: active ? "rgba(201,162,39,0.08)" : "transparent" }}>
                                                            {link.label}
                                                          </Link>Link>
                                                        );
                    })}
