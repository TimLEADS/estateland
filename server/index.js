...
import { appendPayment, upsertRelator, removeRelator } from "./sheets.js";
...
// -- Migrate existing DB: add password column if missing --
try {
...
const SHEETS_READY = !!(process.env.GOOGLE_SERVICE_ACCOUNT_KEY && process.env.GOOGLE_SHEETS_PAYMENTS_ID && process.env.GOOGLE_SHEETS_RELATORS_ID);
if (!SHEETS_READY) {
  console.warn("Google Sheets env vars not set -- sheets sync disabled. Set GOOGLE_SERVICE_ACCOUNT_KEY, GOOGLE_SHEETS_PAYMENTS_ID, GOOGLE_SHEETS_RELATORS_ID to enable.");
}
...
app.post("/api/users", (req, res) => {
  try {
    ...
    db.prepare("INSERT INTO users (id, name, email, phone, brokerage, planId, region, zips, state, county, primaryArea, primarySMR, secondaryArea, secondarySMR, signupDate, documentSignDate, lastLeadSent, leadSentCount, ha, remarks, leadType, note, createdAt, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(
      id, u.name||"", u.email||"", u.phone||"", u.brokerage||"", u.planId||"", u.region||"", u.zips||"", u.state||"", u.county||"", u.primaryArea||"", u.primarySMR||"", u.secondaryArea||"", u.secondarySMR||"", u.signupDate||today, u.documentSignDate||today, u.lastLeadSent||"", u.leadSentCount||0, u.ha||"", u.remarks||"", u.leadType||"", u.note||"", u.createdAt||now, u.password||""
    );
...
app.delete("/api/users/:id", (req, res) => {
  try {
    // Get email before deleting for Sheets removal
    ...
    if (SHEETS_READY && user?.email) removeRelator(user.email).catch(e => console.error("Sheets removeRelator error:", e));
    res.json({ ok: true });
  } catch (err) {
...
app.post("/api/onboarding", (req, res) => {
  try {
    ...
    db.prepare("INSERT INTO onboarding_sessions (id, step, plan, territory, contact, startedAt, lastActivityAt, submittedAt, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").run(
      id, s.step||1, JSON.stringify(s.plan||{}), JSON.stringify(s.territory||{}), JSON.stringify(s.contact||{}), s.startedAt||now, s.lastActivityAt||now, s.submittedAt||"", s.status||"in_progress"
    );
    res.json({ id });
  } catch (err) {
...
app.post("/api/payments", (req, res) => {
  try {
    ...
    // Sync to Google Sheets (non-blocking)
    const full = { id, amountTotal: p.amountTotal||0, currency: p.currency||"usd", customerEmail: p.customerEmail||"", planId: p.planId||"", stripeSessionId: p.stripeSessionId||"", paidAt: p.paidAt||now };
    if (SHEETS_READY) appendPayment(full).catch(e => console.error("Sheets appendPayment error:", e));
    res.json({ id });
  } catch (err) {
