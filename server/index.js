...
app.get("/api/dashboard", (req, res) => {
    try {
      ...
      const onboardingSessions = rawSessions.map(s => ({
          ...
          const chatSessions = rawChats.map(c => ({
                  ...c,
...
app.post("/api/users", (req, res) => {
    try {
          ...
          db.prepare(`INSERT INTO users
                (id, name, email, phone, brokerage, planId, region, zips, state, county,
                  ...
                  u.signupDate||today, u.documentSignDate||today,
                  u.lastLeadSent||"", u.leadSentCount||0,
                  u.ha||"", u.remarks||"", u.leadType||"", u.note||"",
                  u.createdAt||now, u.password||""
                );
          res.json({ id });
    } catch (err) {
...
app.post("/api/onboarding", (req, res) => {
    try {
          ...
          const now = new Date().toISOString();
          db.prepare(`INSERT INTO onboarding_sessions
                (id, step, plan, territory, contact, startedAt, lastActivityAt, submittedAt, status)
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
                  id, s.step||1,
                  JSON.stringify(s.plan||{}), JSON.stringify(s.territory||{}), JSON.stringify(s.contact||{}),
